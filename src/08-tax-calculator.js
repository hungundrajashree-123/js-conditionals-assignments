...
export function calculateTax(income) {
 // Bracket 1: 0% on income up to 10,000
  if (income <= 0) {
    return 0;
  }
  ...
  // Bracket 2: 10% on amount above 10,000 up to 30,000
  if (income > 10000) {
    const taxable = Math.min(income, 30000) - 10000;
    tax += taxable * 0.1;
  }

  // Bracket 3: 20% on amount above 30,000 up to 70,000
  if (income > 30000) {
    const taxable = Math.min(income, 70000) - 30000;
    tax += taxable * 0.2;
  }

  // Bracket 4: 30% on amount above 70,000
  if (income > 70000) {
